/* File:  serial_k.c
 * Comple: gcc -g -Wall -fopenmp -o openmp_k_means.out openmp_k_means.c -lm
...
int main(int argc,char* argv[]){
  ...
  thread_count=strtol( argv[3],NULL,10);
	...
	double cpu_begin = omp_get_wtime();
	...
	printf("There are %d threads,Run time for parallel algorithm is %f s.\n",i,(double)(cpu_end-cpu_begin));
	}
	return 0;
}
/*	Initilize_Data()
      ...
      #pragma omp for
    	for(i=0;i<data_size;i++){
    		...
    		min=distance[i][0];
					...
					// try different schudule. performance
					//
    		for(j=0;j<cluster_k;j++){
    			if (min>=distance[i][j]){
                    min = distance[i][j];
